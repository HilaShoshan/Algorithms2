...
// The algorithms included here: FW, Dijkstra

import java.util.Arrays;
import java.util.PriorityQueue;

public class MatrixGraph_Info {

    private int[][] mat;  // mat of weights on the edges
    private int n;  // matrix dim, number of vertices on the graph
     ...
     */
    MatrixGraph_Info(int[] vertexWeight, boolean mb[][]) {
        this.n = mb.length;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if (mb[i][j] && i != j)
    ...
    MatrixGraph_Info(int[] vertexWeight, int[][] edgeWeight) {
    ...
    /**
     * Dijkstra algorithm: solve the single-source shortest-path problem
     * works on directed and undirected graph
     * with positive weights on the edges only !!
     * complexity time: 𝑂(|𝐸|+|𝑉|log|⁡𝑉|)
     * @return array representing the shortest paths between s to each vertex
     */
    public int[] dijkstra(int s) {
        ...
        for(int i = 0; i < dist.length; i++) {
            dist[i] = infinity;
            pred[i] = -1;
            // add vertex to the priority queue by the distance
        }
        dist[s] = 0;
        return dist;
    }
